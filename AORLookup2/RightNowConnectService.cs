...
namespace AORLookup2
{
    class RightNowConnectService
    {
        ...
        public string GetOrgTerritory(string OrgId, string ActionName)
        {
            List<string> orgs = new List<string>();
            string retval = "";
            String SubReg = "";

            string query = "SELECT o.CustomFields.CO.sub_region_ID FROM Organization o WHERE ID = " + OrgId;


            //  MessageBox.Show(query);

            string[] resultSet = GetRNData("Org", query);


            if (resultSet != null && resultSet.Length > 0)
            {
                orgs = resultSet.ToList();
                foreach (string s in orgs)
                {
                    SubReg = s.Split('~')[0];
                    //   MessageBox.Show("SubReg" + SubReg);
                ...
                if (SubReg != "")
                {
                    ...
                    retval = GetSubRegionInfo(SubReg, ActionName);
                }
            }
            return retval;
        ...
        public string GetSubRegionInfo(string SubRegId, string ActionName)
        {
            List<string> subRegs = new List<string>();
            List<string> sr2 = new List<string>();
            int SubRegint = Int32.Parse(SubRegId);
            string RetValue = "";
            

            string query = "SELECT p.Region, p.RPSM FROM TOA.Sub_Region p WHERE ID= " + SubRegint;
            

            //  MessageBox.Show(query);

            string[] resultSet = GetRNData("SubRegions", query);


            if (resultSet != null && resultSet.Length > 0)
            {
                subRegs = resultSet.ToList();
                foreach (string s in subRegs)
                {
                    if (ActionName == "RPSM")
                    {
                        RetValue = s.Split('~')[1];
        ...
        public string GetRegion(string RegID)
        {
            List<string> regions = new List<string>();
            string reg = "";
            string query = "SELECT o.RegionalDirector FROM TOA.Region o WHERE ID = " + RegID;


            

            string[] resultSet = GetRNData("Region", query);


            if (resultSet != null && resultSet.Length > 0)
            {
                regions = resultSet.ToList();
        ...
        public string GetAccount(string acctID)
        {
            List<string> regions = new List<string>();
            string reg = "";
            string query = "SELECT Account FROM Account  WHERE ID = " + Int32.Parse(acctID);


            

            string[] resultSet = GetRNData("Acct", query);
